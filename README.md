...
```bash
# Build the image from the standard Dockerfile name
docker build -t cheetahos-buildenv ./buildenv

# Or build explicitly from the lowercase file
docker build -f buildenv/dockerfile -t cheetahos-buildenv .

# Open a shell in the project build environment
docker run --rm -it -v "$PWD":/workspace -w /workspace cheetahos-buildenv
```
